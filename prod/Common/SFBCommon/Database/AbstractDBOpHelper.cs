...
namespace SFBCommon.Database
{
    ...
    abstract class AbstractDBOpHelper : IDisposable
    {
        ...
        public bool ModifyColumnType(string strTableName, string strFieldName, string strFieldType)
        {
            // Check and make standard SQL command parameters
            if (CheckAndMakeStandardSQLCommandParams(EMSFB_DBPARAMTYPE.emDBParam_TableName, ref strTableName) &&
                CheckAndMakeStandardSQLCommandParams(EMSFB_DBPARAMTYPE.emDBParam_FieldName, ref strFieldName) &&
                CheckAndMakeStandardSQLCommandParams(EMSFB_DBPARAMTYPE.emDBParam_FieldType, ref strFieldType)
                )
            {
                string strSqlCommand = GetModifyColumnTypeSqlCommand(strTableName, strFieldName, strFieldType);
                if (!String.IsNullOrEmpty(strSqlCommand))
                {
                    theLog.OutputLog(EMSFB_LOGLEVEL.emLogLevelDebug, "ModifyColumnType command: [{0}]", strSqlCommand);
                    return ExecuteSQLNoQueryCommand(strSqlCommand);
                }
            }
            else
                        ...
                        if (!string.IsNullOrEmpty(strFullSearchCommand))
                        {
                            using (DbDataAdapter obDbDataAdapter = CreateDbDataAdapter(strFullSearchCommand))
                            {
                                if (null != obDbDataAdapter)
                                {
                                    ...
                                    string sourceTableName = kstrDefaultTableName;
                                    if(lsSearchScopes.Count == 1)
                                    {
                                        sourceTableName = lsSearchScopes.First().ToString();
                                    }
                                    using (DataSet obDataSet = CreateDataSet(obDbDataAdapter, sourceTableName))
                                    {
                                        if (null != obDataSet)
                                        {
                                            return obDataSet.Tables[sourceTableName];
                                        }
                                    }
                                }
                            }
        ...
        abstract protected string GetModifyColumnTypeSqlCommand(string strTableName, string strFieldName, string strFieldType);
