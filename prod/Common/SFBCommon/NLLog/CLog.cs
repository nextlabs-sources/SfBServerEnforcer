    ...
    public class CLog
    {
        ...
        static internal void Init(EMSFB_MODULE emModule)
        {
            try
            {
                ...
                {
                    OutputTraceLog("LogInit: cannot find log config files. SFB install path is:[{0}]\n", CommonHelper.kstrSFBInstallPath);
                }
            }
            catch (Exception ex)
            {
        ...
        static private void InnerInit(string strCfgFileName)  // when we want to use the CLog, we need initialize it by log format configure file first.
        {
            try
            {
                OutputTraceLog("Load Log config file from:{0}", strCfgFileName);
        ...
        public void OutputLog(EMSFB_LOGLEVEL emLogLevel, string strLogInfo)
        {
            OutputLog(emLogLevel, "{0}", strLogInfo);
        }
