...
namespace nextlabs
{
  ...
  class cesdk_connection : boost::noncopyable
  {
	...
	memset(&connHandle,0x00,sizeof(connHandle));
      ...
      void set_sdk( __in nextlabs::cesdk_loader* in_cesdk ) throw()
      {
	...
	cesdk = in_cesdk;
       ...
       */
      void set_timeout( __in size_t in_timeout ) throw()
      {
	timeout = (CEint32)in_timeout;
       ...
       */
      __checkReturn bool connect(void) throw()
      {
	CEApplication app;
	CEUser user;
	...
	return last_error == CE_RESULT_SUCCESS;
       ...
       *         is empty.
       */
      void set_app_path( __in const wchar_t* in_app_path )
      {
