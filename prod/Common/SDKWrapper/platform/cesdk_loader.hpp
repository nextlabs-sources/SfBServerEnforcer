...
namespace nextlabs
{
  ...
  typedef CEResult_t(*CEEVALUATE_CheckResourcesEx_t)
      (CEHandle handle,
      CERequest *requests,
      CEint32 numRequests,
  ...
  class cesdk_loader
  {
    ...
    virtual bool load( __in const wchar_t* path ) throw()
    {
      ...
      if( path == NULL )
      {
          ::OutputDebugStringW(L"faild error path \n");
        return false;
      }
...
#if defined(_M_IX86)
      _snwprintf_s(libpath,_countof(libpath),L"%s\\cesdk32.dll",path);
#elif defined(_M_X64)
      _snwprintf_s(libpath,_countof(libpath),L"%s\\cesdk.dll",path);
#endif
      OutputDebugStringW(L"----Path begin ----");
      OutputDebugStringW(libpath);
      OutputDebugStringW(L"----Path end ----");

      hlib = LoadLibraryW(libpath);
      if( hlib == NULL )
      {
          ::OutputDebugStringW(L"load cesdk or cesdk32 failed \n");
        return false;
      }
      ...
      if (NULL == fns.CEEVALUATE_CheckResourcesEx)
      {
          ::OutputDebugStringW(L"Old interface, no check_resourceex interface \n");
      }
      ...
      /*fns.CEEVALUATE_CheckResourcesEx == NULL ||*/
