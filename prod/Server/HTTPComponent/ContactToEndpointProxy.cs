...
namespace Nextlabs.SFBServerEnforcer.HTTPComponent
{
    ...
    class ContactToEndpointProxy
    {
        ...
        protected ContactToEndpointProxy(EMSFB_ENDPOINTTTYPE emEndpointType) 
        {
            ...
            try
            {
                STUSFB_ENDPOINTPROXYTCPINFO stuEndpointProxyTcpInfo = CConfig.GetEndpointProxyTcpInfo(emEndpointType);
        ...
        public void SendMessageToUser(string strNotifyXml, HttpChatRoomInfo obChatRoomInfo, string strInviter, string strInvitee, string strPolicyName, NLChatRoomAttachmentInfo obChatRoomAttachmentInfo)
        {
            //construct diction
            Dictionary<string, string> dicWildcard = new Dictionary<string, string>();

            List<string> lstDistinctParticipant = null;
            if (null != obChatRoomInfo)
            {
                lstDistinctParticipant = obChatRoomInfo.GetParticipants();

                dicWildcard.Add(CommandHelper.kstrWildcardCreator, obChatRoomInfo.Creator);
                dicWildcard.Add(CommandHelper.kstrWildcardSfbObjUri, obChatRoomInfo.ChatRoomID);
                dicWildcard.Add(CommandHelper.kstrWildcardChatRoomName, obChatRoomInfo.Name);
                dicWildcard.Add(CommandHelper.kstrWildcardChatRoomMembers, SFBCommon.Common.CommonHelper.GetSolidString(obChatRoomInfo.Members));   // For open chat room, the members property maybe null
                dicWildcard.Add(CommandHelper.kstrWildcardChatRoomManagers, SFBCommon.Common.CommonHelper.GetSolidString(obChatRoomInfo.Managers)); // Not all chat room has managers, the managers property maybe null
                dicWildcard.Add(CommandHelper.kstrWildcardNLClassifyTagsName, obChatRoomInfo.sfbRoomVar.GetStringTags());
            }
            ...
            if (null != obChatRoomAttachmentInfo)
            {
                dicWildcard.Add(CommandHelper.kstrWildcardFileName, obChatRoomAttachmentInfo.GetItemValue(NLChatRoomAttachmentInfo.kstrFileOrgNameFieldName));
                dicWildcard.Add(CommandHelper.kstrWildcardFileOwner, obChatRoomAttachmentInfo.GetItemValue(NLChatRoomAttachmentInfo.kstrFileOwnerFieldName));
                dicWildcard.Add(CommandHelper.kstrWildcardFileTag, obChatRoomAttachmentInfo.GetStringTags());
            }
        ...
        private Dictionary<string, string> GetWildcardAnchorValue(List<string> lstWildcardAnchor, List<string> lstParticipate)
        {
            ...
            int nPolicyQueryResult = HttpPolicy.GetInstance().QueryPolicyForWildcardAnchorMulti(arrayBaseRequestData, out arrayPolicyResult);
