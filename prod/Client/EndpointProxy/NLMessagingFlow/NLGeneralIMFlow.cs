            ...
            */
            if ((null != obIMFlow) && (!string.IsNullOrEmpty(strReveivedMessageBody)))
            {
                ...
                else
                {
                    ...
                    const string kstrDefaultErrorReply = "System error, please contact system administrator for help.";

                    string strAutoReplyMessage = "";
                    string strRemoteUserSipUri = GetRemoteChatSpeakerUri();
                    ...
                    {
                        strAutoReplyMessage = EndpointProxyCommonTools.GetAgentAutoReplyInfo(NLLyncEndpointProxyConfigInfo.s_endpointProxyConfigInfo.AgentAutoReply, strRemoteUserSipUri);
                    }
                    else if (EMSFB_ENDPOINTTTYPE.emTypeAssistant == emEndpointType)
                    {
                        bool bAllSuccess = false;
                        strAutoReplyMessage = EndpointProxyCommonTools.GetClassifyMeetingAutoReplyInfo(NLLyncEndpointProxyConfigInfo.s_endpointProxyConfigInfo.AssitantAutoReply, strRemoteUserSipUri, "", out bAllSuccess);
                    }
                    if (string.IsNullOrEmpty(strAutoReplyMessage))
                    {
                        strAutoReplyMessage = kstrDefaultErrorReply;
                    }
                    UCMAHelper.SendMessage(obIMFlow, strAutoReplyMessage, CBSendOutMessageComplete);
                }
            }
        }
        #endregion
